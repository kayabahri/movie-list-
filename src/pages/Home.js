      ...
      {trendingMovie && (
        <div
          ...
          style={{
            ...
            height: 'calc(100vh - 365px)',  // Çizgi ile hizalanması için yüksekliği ayarlandı
            filter: 'brightness(0.2) opacity(0.5)',
          }}
        ></div>
      )}
      <div className="container mx-auto px-1 py-8 relative z-10"> {/* Padding daha da azaltıldı */}
        ...
        <Slider {...settings}>
          {movies.map((movie, index) => (
            <div key={index} className="px-0.5"> {/* Padding azaltıldı */}
              <Link to={`/movie/${movie.id}`} className="block max-w-xs mx-auto"> {/* Genişlik sınırlandı */}
                <div className="relative rounded-lg overflow-hidden group">
                  <div className="relative overflow-hidden rounded-lg">
                    <img 
                      src={`${baseURL}${movie.poster_path}`} 
                      alt={movie.title} 
                      className="w-full h-56 md:h-72 lg:h-96 object-cover rounded-lg transition-transform duration-300 group-hover:scale-110" 
                    />
                  ...
                  <div className="p-2 bg-gray-900 bg-opacity-5"> {/* Padding azaltıldı */}
